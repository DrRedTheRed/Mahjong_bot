...
def init_yama():
    # 声明全局变量 yama，以便在函数内部修改它
    global yama
    
    # 初始化 yama 为空列表，这个列表将用来存储牌墙中的所有牌
    yama = []
    
    # CHUN 是一个常量，代表麻将中的一种花色的数量加上字牌
    # 例如，如果万、条、筒各有9种，字牌有7种，那么 CHUN 应该是 9 + 7
    # 循环从0开始，到CHUN（包含）结束
    for h in range(CHUN+1):
        # 对于每种牌值，每种牌都有4张
        for _ in range(4):
            # 将当前的牌值 h 添加到 yama 列表中，表示牌墙中有这张牌
            yama.append(h)
    
    # 使用 random 模块的 shuffle 函数随机打乱 yama 列表中的元素顺序
    # 这样可以模拟洗牌的过程，确保每次游戏开始时牌的顺序都是随机的
    random.shuffle(yama)
...
def get_outer_tsumo(hai_str):

    if get_hai_number(hai_str) >=0:
        return get_hai_number(hai_str)
    else:
        return -1

def get_outer_haipai(hai_str):
    # 使用正则表达式匹配所有项
    items = re.findall(r'\d+[A-Za-z]|[\u4e00-\u9fff]', hai_str)
    haipai = [0 for i in range(CHUN+1)]
    # 使用 for 循环每次迭代两个元素
    for i in range(0, len(items), 1):
        # 检查是否有足够的元素来形成一对
        if i  < len(items):
            pair = items[i]  # 获取当前索引和下一个索引的元素
            h = get_hai_number(pair)
            haipai[h] += 1
    return haipai

def replace_and_find_position(str, tsumo, result_str):
    # 使用正则表达式匹配所有项
    ...
    if str in items:
        # 找到第一个匹配项的索引
        position = items.index(str) + 1  # 因为索引从0开始，所以加1
        # 从列表中删除第一个匹配项，并插入tsumo
        items[position - 1:position] = [tsumo]  # 替换str为tsumo
        # 将更新后的列表重新组合成字符串
...
if __name__ == '__main__':
    info = AgariInfo(get_tehai_from_string("東東南南中中中発発発白白白"), TON)
    print(info.is_agari())
